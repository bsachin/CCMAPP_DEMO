...
DATA LTRL65C0R1240 TYPE TDT_VBUK.
DATA LTL65C0R4109 TYPE VBUK_KEY_TAB.
LOOP AT LT_VBUK REFERENCE INTO DATA(LDL65C0R9499).
  INSERT VALUE #( VBELN = LDL65C0R9499->VBELN ) INTO TABLE LTL65C0R4109.
ENDLOOP.
CALL FUNCTION 'SD_VBUK_READ_FROM_DOC_MULTI'
  EXPORTING
    IT_VBUK_KEY         = LTL65C0R4109
  IMPORTING
    ET_VBUK             = LTRL65C0R1240
  EXCEPTIONS
    VBELN_NOT_FOUND     = 1
    VBTYP_NOT_SUPPORTED = 2
    VBOBJ_NOT_SUPPORTED = 3
    OTHERS              = 4.
IF SY-SUBRC = 0 AND LINES( LTRL65C0R1240 ) > 0.
  CLEAR S_VBUK.
  TYPES: BEGIN OF TYL65C0R2666,
  ...
  END OF TYL65C0R2666.
  DATA: LML65C0R2134 TYPE TYL65C0R2666,
        LWL65C0R869 LIKE LINE OF S_VBUK.
  LOOP AT LTRL65C0R1240 REFERENCE INTO DATA(LDRL65C0R3319).
    LML65C0R2134-VBELN = LDRL65C0R3319->VBELN.
    LML65C0R2134-UVALL = LDRL65C0R3319->UVALL.
    LML65C0R2134-LSSTK = LDRL65C0R3319->LSSTK.
    LML65C0R2134-FSSTK = LDRL65C0R3319->FSSTK.
    LML65C0R2134-CMGST = LDRL65C0R3319->CMGST.
    LML65C0R2134-BESTK = LDRL65C0R3319->BESTK.
    LML65C0R2134-LFGSK = LDRL65C0R3319->LFGSK.
    LML65C0R2134-LFSTK = LDRL65C0R3319->LFSTK.
    LML65C0R2134-FKSTK = LDRL65C0R3319->FKSTK.
    LML65C0R2134-SPSTG = LDRL65C0R3319->SPSTG.
    LML65C0R2134-TRSTA = LDRL65C0R3319->TRSTA.
    LML65C0R2134-WBSTK = LDRL65C0R3319->WBSTK.
    LML65C0R2134-GBSTK = LDRL65C0R3319->GBSTK.
    LML65C0R2134-COSTA = LDRL65C0R3319->COSTA.
    LWL65C0R869 = LML65C0R2134.
    APPEND LWL65C0R869 TO S_VBUK.
  ENDLOOP.
  SY-DBCNT = LINES( LTRL65C0R1240 ).
ELSE.
  SY-SUBRC = 4.
  SY-DBCNT = 0.
ENDIF.
...
CALL FUNCTION 'SD_VBUK_READ_FROM_DOC'
  ...
  EXCEPTIONS
    ...
    OTHERS              = 4.
IF SY-SUBRC = 0.
  L_LFSTK = LL76C0R7732-LFSTK.
  L_UVPIS = LL76C0R7732-UVPIS.
  SY-DBCNT = 1.
ELSE.
  SY-SUBRC = 4.
  SY-DBCNT = 0.
...
CALL FUNCTION 'SD_VBUK_READ_FROM_DOC'
  ...
  EXCEPTIONS
    ...
    OTHERS              = 4.
IF SY-SUBRC = 0.
  TYPES: BEGIN OF TYL80C0R1505,
    LFSTK TYPE VBUK-LFSTK,
    UVPIS TYPE VBUK-UVPIS,
  END OF TYL80C0R1505.
  DATA LML80C0R261 TYPE TYL80C0R1505.
  LML80C0R261-LFSTK = LL80C0R6423-LFSTK.
  LML80C0R261-UVPIS = LL80C0R6423-UVPIS.
  MOVE-CORRESPONDING LML80C0R261 TO LS_VBUK.
  SY-DBCNT = 1.
ELSE.
  SY-SUBRC = 4.
  SY-DBCNT = 0.
...
DATA LSL116C0R7165 TYPE VBUP.
CALL FUNCTION 'SD_VBUP_READ_FROM_DOC'
  EXPORTING
    I_VBELN             = '0000000001'
    I_POSNR             = '000007'
  IMPORTING
    ES_VBUP             = LSL116C0R7165
  EXCEPTIONS
    VBELN_NOT_FOUND     = 1
    VBTYP_NOT_SUPPORTED = 2
    VBOBJ_NOT_SUPPORTED = 3
    OTHERS              = 4.
IF SY-SUBRC = 0.
  LS_VBUP-FKSTA = LSL116C0R7165-FKSTA.
  LS_VBUP-FKSAA = LSL116C0R7165-FKSAA.
  SY-DBCNT = 1.
ELSE.
  SY-SUBRC = 4.
  SY-DBCNT = 0.
...
DATA LSL122C0R2857 TYPE VBUP.
CALL FUNCTION 'SD_VBUP_READ_FROM_DOC'
  EXPORTING
    I_VBELN             = '0000000001'
    I_POSNR             = '000007'
  IMPORTING
    ES_VBUP             = LSL122C0R2857
  EXCEPTIONS
    VBELN_NOT_FOUND     = 1
    VBTYP_NOT_SUPPORTED = 2
    VBOBJ_NOT_SUPPORTED = 3
    OTHERS              = 4.
IF SY-SUBRC = 0.
  TYPES: BEGIN OF TYL122C0R9227,
    FKSTA TYPE VBUP-FKSTA,
    FKSAA TYPE VBUP-FKSAA,
  END OF TYL122C0R9227.
  DATA LML122C0R9224 TYPE TYL122C0R9227.
  LML122C0R9224-FKSTA = LSL122C0R2857-FKSTA.
  LML122C0R9224-FKSAA = LSL122C0R2857-FKSAA.
  MOVE-CORRESPONDING LML122C0R9224 TO LS_VBUP.
  SY-DBCNT = 1.
ELSE.
  SY-SUBRC = 4.
  SY-DBCNT = 0.
...
DATA LTL136C0R1346 TYPE TABLE OF VBUP.
CALL FUNCTION 'SD_VBUP_READ_FROM_DOC'
  EXPORTING
    I_VBELN             = '0000000001'
  IMPORTING
    ET_VBUP             = LTL136C0R1346
  EXCEPTIONS
    VBELN_NOT_FOUND     = 1
    VBTYP_NOT_SUPPORTED = 2
    VBOBJ_NOT_SUPPORTED = 3
    OTHERS              = 4.
IF SY-SUBRC = 0 AND LINES( LTL136C0R1346 ) > 0.
  CLEAR LT_VBUP.
  TYPES: BEGIN OF TYL136C0R5193,
    VBELN TYPE VBUP-VBELN,
    POSNR TYPE VBUP-POSNR,
  END OF TYL136C0R5193.
  DATA: LML136C0R518 TYPE TYL136C0R5193,
        LWL136C0R6859 LIKE LINE OF LT_VBUP.
  LOOP AT LTL136C0R1346 REFERENCE INTO DATA(LDRL136C0R3774).
    LML136C0R518-VBELN = LDRL136C0R3774->VBELN.
    LML136C0R518-POSNR = LDRL136C0R3774->POSNR.
    MOVE-CORRESPONDING LML136C0R518 TO LWL136C0R6859.
    APPEND LWL136C0R6859 TO LT_VBUP.
  ENDLOOP.
  SY-DBCNT = LINES( LTL136C0R1346 ).
ELSE.
  SY-SUBRC = 4.
  SY-DBCNT = 0.
ENDIF.
