...
DATA LTRL66C0R8119 TYPE TDT_VBUK.
DATA LTL66C0R2646 TYPE VBUK_KEY_TAB.
LOOP AT LT_VBUK REFERENCE INTO DATA(LDL66C0R5894).
  INSERT VALUE #( VBELN = LDL66C0R5894->VBELN ) INTO TABLE LTL66C0R2646.
ENDLOOP.
CALL FUNCTION 'SD_VBUK_READ_FROM_DOC_MULTI'
  EXPORTING
    IT_VBUK_KEY         = LTL66C0R2646
  IMPORTING
    ET_VBUK             = LTRL66C0R8119
  EXCEPTIONS
    VBELN_NOT_FOUND     = 1
    VBTYP_NOT_SUPPORTED = 2
    VBOBJ_NOT_SUPPORTED = 3
    OTHERS              = 4.
IF SY-SUBRC = 0 AND LINES( LTRL66C0R8119 ) > 0.
  CLEAR S_VBUK.
  TYPES: BEGIN OF TYL66C0R2765,
  ...
  END OF TYL66C0R2765.
  DATA: LML66C0R219 TYPE TYL66C0R2765,
        LWL66C0R9065 LIKE LINE OF S_VBUK.
  LOOP AT LTRL66C0R8119 REFERENCE INTO DATA(LDRL66C0R6136).
    LML66C0R219-VBELN = LDRL66C0R6136->VBELN.
    LML66C0R219-UVALL = LDRL66C0R6136->UVALL.
    LML66C0R219-LSSTK = LDRL66C0R6136->LSSTK.
    LML66C0R219-FSSTK = LDRL66C0R6136->FSSTK.
    LML66C0R219-CMGST = LDRL66C0R6136->CMGST.
    LML66C0R219-BESTK = LDRL66C0R6136->BESTK.
    LML66C0R219-LFGSK = LDRL66C0R6136->LFGSK.
    LML66C0R219-LFSTK = LDRL66C0R6136->LFSTK.
    LML66C0R219-FKSTK = LDRL66C0R6136->FKSTK.
    LML66C0R219-SPSTG = LDRL66C0R6136->SPSTG.
    LML66C0R219-TRSTA = LDRL66C0R6136->TRSTA.
    LML66C0R219-WBSTK = LDRL66C0R6136->WBSTK.
    LML66C0R219-GBSTK = LDRL66C0R6136->GBSTK.
    LML66C0R219-COSTA = LDRL66C0R6136->COSTA.
    LWL66C0R9065 = LML66C0R219.
    APPEND LWL66C0R9065 TO S_VBUK.
  ENDLOOP.
  SY-DBCNT = LINES( LTRL66C0R8119 ).
ELSE.
  SY-SUBRC = 4.
  SY-DBCNT = 0.
ENDIF.
...
CALL FUNCTION 'SD_VBUK_READ_FROM_DOC'
  ...
  EXCEPTIONS
    ...
    OTHERS              = 4.
IF SY-SUBRC = 0.
  L_LFSTK = LL77C0R2508-LFSTK.
  L_UVPIS = LL77C0R2508-UVPIS.
  SY-DBCNT = 1.
ELSE.
  SY-SUBRC = 4.
  SY-DBCNT = 0.
...
CALL FUNCTION 'SD_VBUK_READ_FROM_DOC'
  ...
  EXCEPTIONS
    ...
    OTHERS              = 4.
IF SY-SUBRC = 0.
  TYPES: BEGIN OF TYL81C0R8237,
    LFSTK TYPE VBUK-LFSTK,
    UVPIS TYPE VBUK-UVPIS,
  END OF TYL81C0R8237.
  DATA LML81C0R1048 TYPE TYL81C0R8237.
  LML81C0R1048-LFSTK = LL81C0R3119-LFSTK.
  LML81C0R1048-UVPIS = LL81C0R3119-UVPIS.
  MOVE-CORRESPONDING LML81C0R1048 TO LS_VBUK.
  SY-DBCNT = 1.
ELSE.
  SY-SUBRC = 4.
  SY-DBCNT = 0.
...
DATA LSL125C0R3785 TYPE VBUP.
CALL FUNCTION 'SD_VBUP_READ_FROM_DOC'
  EXPORTING
    I_VBELN             = '0000000001'
    I_POSNR             = '000007'
  IMPORTING
    ES_VBUP             = LSL125C0R3785
  EXCEPTIONS
    VBELN_NOT_FOUND     = 1
    VBTYP_NOT_SUPPORTED = 2
    VBOBJ_NOT_SUPPORTED = 3
    OTHERS              = 4.
IF SY-SUBRC = 0.
  LS_VBUP-FKSTA = LSL125C0R3785-FKSTA.
  LS_VBUP-FKSAA = LSL125C0R3785-FKSAA.
  SY-DBCNT = 1.
ELSE.
  SY-SUBRC = 4.
  SY-DBCNT = 0.
...
DATA LSL132C0R4189 TYPE VBUP.
CALL FUNCTION 'SD_VBUP_READ_FROM_DOC'
  EXPORTING
    I_VBELN             = '0000000001'
    I_POSNR             = '000007'
  IMPORTING
    ES_VBUP             = LSL132C0R4189
  EXCEPTIONS
    VBELN_NOT_FOUND     = 1
    VBTYP_NOT_SUPPORTED = 2
    VBOBJ_NOT_SUPPORTED = 3
    OTHERS              = 4.
IF SY-SUBRC = 0.
  TYPES: BEGIN OF TYL132C0R4951,
    FKSTA TYPE VBUP-FKSTA,
    FKSAA TYPE VBUP-FKSAA,
  END OF TYL132C0R4951.
  DATA LML132C0R8337 TYPE TYL132C0R4951.
  LML132C0R8337-FKSTA = LSL132C0R4189-FKSTA.
  LML132C0R8337-FKSAA = LSL132C0R4189-FKSAA.
  MOVE-CORRESPONDING LML132C0R8337 TO LS_VBUP.
  SY-DBCNT = 1.
ELSE.
  SY-SUBRC = 4.
  SY-DBCNT = 0.
...
DATA LTL146C0R1402 TYPE TABLE OF VBUP.
CALL FUNCTION 'SD_VBUP_READ_FROM_DOC'
  EXPORTING
    I_VBELN             = '0000000001'
  IMPORTING
    ET_VBUP             = LTL146C0R1402
  EXCEPTIONS
    VBELN_NOT_FOUND     = 1
    VBTYP_NOT_SUPPORTED = 2
    VBOBJ_NOT_SUPPORTED = 3
    OTHERS              = 4.
IF SY-SUBRC = 0 AND LINES( LTL146C0R1402 ) > 0.
  CLEAR LT_VBUP.
  TYPES: BEGIN OF TYL146C0R6099,
    VBELN TYPE VBUP-VBELN,
    POSNR TYPE VBUP-POSNR,
  END OF TYL146C0R6099.
  DATA: LML146C0R7318 TYPE TYL146C0R6099,
        LWL146C0R327 LIKE LINE OF LT_VBUP.
  LOOP AT LTL146C0R1402 REFERENCE INTO DATA(LDRL146C0R9206).
    LML146C0R7318-VBELN = LDRL146C0R9206->VBELN.
    LML146C0R7318-POSNR = LDRL146C0R9206->POSNR.
    MOVE-CORRESPONDING LML146C0R7318 TO LWL146C0R327.
    APPEND LWL146C0R327 TO LT_VBUP.
  ENDLOOP.
  SY-DBCNT = LINES( LTL146C0R1402 ).
ELSE.
  SY-SUBRC = 4.
  SY-DBCNT = 0.
ENDIF.
...
DATA LL174C0R1656 TYPE VBUK.
CALL FUNCTION 'SD_VBUK_READ_FROM_DOC'
  EXPORTING
    I_VBELN             = L_VBELN
  IMPORTING
    ES_VBUK             = LL174C0R1656
  EXCEPTIONS
    VBELN_NOT_FOUND     = 1
    VBTYP_NOT_SUPPORTED = 2
    VBOBJ_NOT_SUPPORTED = 3
    OTHERS              = 4.
IF SY-SUBRC = 0.
  TYPES: BEGIN OF TYL174C0R4168,
    BUCHK TYPE VBUK-BUCHK,
    ABSTK TYPE VBUK-ABSTK,
    UVPAS TYPE VBUK-UVPAS,
  END OF TYL174C0R4168.
  DATA LML174C0R4727 TYPE TYL174C0R4168.
  LML174C0R4727-BUCHK = LL174C0R1656-BUCHK.
  LML174C0R4727-ABSTK = LL174C0R1656-ABSTK.
  LML174C0R4727-UVPAS = LL174C0R1656-UVPAS.
  MOVE-CORRESPONDING LML174C0R4727 TO VBUK.
  SY-DBCNT = 1.
ELSE.
  SY-SUBRC = 4.
  SY-DBCNT = 0.
...
DATA LL199C0R3144 TYPE VBUK.
CALL FUNCTION 'SD_VBUK_READ_FROM_DOC'
  EXPORTING
    I_VBELN             = L_VBELN
    I_VBOBJ             = 'L'
  IMPORTING
    ES_VBUK             = LL199C0R3144
  EXCEPTIONS
    VBELN_NOT_FOUND     = 1
    VBTYP_NOT_SUPPORTED = 2
    VBOBJ_NOT_SUPPORTED = 3
...
DATA LTL218C0R6382 TYPE VBUK_KEY_TAB.
LOOP AT LT_KEY REFERENCE INTO DATA(LDL218C0R5048).
  INSERT VALUE #( VBELN = LDL218C0R5048->* ) INTO TABLE LTL218C0R6382.
ENDLOOP.
CALL FUNCTION 'SD_VBUK_READ_FROM_DOC_MULTI'
  EXPORTING
    IT_VBUK_KEY         = LTL218C0R6382
  IMPORTING
    ET_VBUK             = LT_RESULT
  EXCEPTIONS
    VBELN_NOT_FOUND     = 1
...
DATA LL254C0R9215 TYPE VBUK.
CALL FUNCTION 'SD_VBUK_READ_FROM_DOC'
  EXPORTING
    I_VBELN             = '00001'
  IMPORTING
    ES_VBUK             = LL254C0R9215
  EXCEPTIONS
    VBELN_NOT_FOUND     = 1
    VBTYP_NOT_SUPPORTED = 2
    VBOBJ_NOT_SUPPORTED = 3
    OTHERS              = 4.
IF SY-SUBRC = 0.
  LS_VBUK-ABSTK = LL254C0R9215-ABSTK.
  LS_VBUK-COSTA = LL254C0R9215-COSTA.
  LS_VBUK-WBSTK = LL254C0R9215-WBSTK.
  SY-DBCNT = 1.
ELSE.
  SY-SUBRC = 4.
  SY-DBCNT = 0.
...
DATA LL258C0R3309 TYPE VBUK.
CALL FUNCTION 'SD_VBUK_READ_FROM_DOC'
  EXPORTING
    I_VBELN             = '0000000002'
  IMPORTING
    ES_VBUK             = LL258C0R3309
  EXCEPTIONS
    VBELN_NOT_FOUND     = 1
    VBTYP_NOT_SUPPORTED = 2
    VBOBJ_NOT_SUPPORTED = 3
    OTHERS              = 4.
IF SY-SUBRC = 0.
  L_LFSTK = LL258C0R3309-LFSTK.
  L_UVPIS = LL258C0R3309-UVPIS.
  SY-DBCNT = 1.
ELSE.
  SY-SUBRC = 4.
  SY-DBCNT = 0.
...
DATA LL263C0R5439 TYPE VBUK.
CALL FUNCTION 'SD_VBUK_READ_FROM_DOC'
  EXPORTING
    I_VBELN             = '0000000002'
  IMPORTING
    ES_VBUK             = LL263C0R5439
  EXCEPTIONS
    VBELN_NOT_FOUND     = 1
    VBTYP_NOT_SUPPORTED = 2
    VBOBJ_NOT_SUPPORTED = 3
    OTHERS              = 4.
IF SY-SUBRC = 0.
  TYPES: BEGIN OF TYL263C0R2701,
    LFSTK TYPE VBUK-LFSTK,
    UVPIS TYPE VBUK-UVPIS,
  END OF TYL263C0R2701.
  DATA LML263C0R354 TYPE TYL263C0R2701.
  LML263C0R354-LFSTK = LL263C0R5439-LFSTK.
  LML263C0R354-UVPIS = LL263C0R5439-UVPIS.
  MOVE-CORRESPONDING LML263C0R354 TO LS_VBUK.
  SY-DBCNT = 1.
ELSE.
  SY-SUBRC = 4.
  SY-DBCNT = 0.
...
DATA LTL282C0R7289 TYPE VBUK_KEY_TAB.
LOOP AT LT_KEY REFERENCE INTO DATA(LDL282C0R881).
  INSERT VALUE #( VBELN = LDL282C0R881->* ) INTO TABLE LTL282C0R7289.
ENDLOOP.
CALL FUNCTION 'SD_VBUK_READ_FROM_DOC_MULTI'
  EXPORTING
    IT_VBUK_KEY         = LTL282C0R7289
  IMPORTING
    ET_VBUK             = LT_RESULT
  EXCEPTIONS
    VBELN_NOT_FOUND     = 1
...
DATA LTL290C0R9268 TYPE VBUK_KEY_TAB.
LOOP AT LT_VBUK REFERENCE INTO DATA(LDL290C0R8136).
  INSERT VALUE #( VBELN = LDL290C0R8136->VBELN ) INTO TABLE LTL290C0R9268.
ENDLOOP.
CALL FUNCTION 'SD_VBUK_READ_FROM_DOC_MULTI'
  EXPORTING
    IT_VBUK_KEY         = LTL290C0R9268
  IMPORTING
    ET_VBUK             = LT_RESULT
  EXCEPTIONS
    VBELN_NOT_FOUND     = 1
...
DATA LL294C0R7955 TYPE VBUK.
CALL FUNCTION 'SD_VBUK_READ_FROM_DOC'
  EXPORTING
    I_VBELN             = '01'
  IMPORTING
    ES_VBUK             = LL294C0R7955
  EXCEPTIONS
    VBELN_NOT_FOUND     = 1
    VBTYP_NOT_SUPPORTED = 2
    VBOBJ_NOT_SUPPORTED = 3
    OTHERS              = 4.
IF SY-SUBRC = 0.
  TYPES: BEGIN OF TYL294C0R4570,
    VBELN TYPE VBUK-VBELN,
    UVALL TYPE VBUK-UVALL,
  END OF TYL294C0R4570.
  DATA LML294C0R6916 TYPE TYL294C0R4570.
  LML294C0R6916-VBELN = LL294C0R7955-VBELN.
  LML294C0R6916-UVALL = LL294C0R7955-UVALL.
  LS_VBUK = LML294C0R6916.
  SY-DBCNT = 1.
ELSE.
  SY-SUBRC = 4.
  SY-DBCNT = 0.
...
DATA LTRL303C0R5133 TYPE TDT_VBUK.
DATA LTL303C0R3013 TYPE VBUK_KEY_TAB.
LOOP AT LT_VBUK REFERENCE INTO DATA(LDL303C0R3925).
  INSERT VALUE #( VBELN = LDL303C0R3925->VBELN ) INTO TABLE LTL303C0R3013.
ENDLOOP.
CALL FUNCTION 'SD_VBUK_READ_FROM_DOC_MULTI'
  EXPORTING
    IT_VBUK_KEY         = LTL303C0R3013
  IMPORTING
    ET_VBUK             = LTRL303C0R5133
  EXCEPTIONS
    VBELN_NOT_FOUND     = 1
    VBTYP_NOT_SUPPORTED = 2
    VBOBJ_NOT_SUPPORTED = 3
    OTHERS              = 4.
IF SY-SUBRC = 0 AND LINES( LTRL303C0R5133 ) > 0.
  LT_TARGET = LTRL303C0R5133.
  SY-DBCNT = LINES( LTRL303C0R5133 ).
ELSE.
  SY-SUBRC = 4.
  SY-DBCNT = 0.
ENDIF.
...
DATA LSL345C0R5367 TYPE VBUP.
CALL FUNCTION 'SD_VBUP_READ_FROM_DOC'
  EXPORTING
    I_VBELN             = '0000000001'
    I_POSNR             = '000007'
  IMPORTING
    ES_VBUP             = LSL345C0R5367
  EXCEPTIONS
    VBELN_NOT_FOUND     = 1
    VBTYP_NOT_SUPPORTED = 2
    VBOBJ_NOT_SUPPORTED = 3
    OTHERS              = 4.
IF SY-SUBRC = 0.
  LS_VBUP-FKSTA = LSL345C0R5367-FKSTA.
  LS_VBUP-FKSAA = LSL345C0R5367-FKSAA.
  SY-DBCNT = 1.
ELSE.
  SY-SUBRC = 4.
  SY-DBCNT = 0.
...
DATA LSL352C0R387 TYPE VBUP.
CALL FUNCTION 'SD_VBUP_READ_FROM_DOC'
  EXPORTING
    I_VBELN             = '0000000001'
    I_POSNR             = '000007'
  IMPORTING
    ES_VBUP             = LSL352C0R387
  EXCEPTIONS
    VBELN_NOT_FOUND     = 1
    VBTYP_NOT_SUPPORTED = 2
    VBOBJ_NOT_SUPPORTED = 3
    OTHERS              = 4.
IF SY-SUBRC = 0.
  TYPES: BEGIN OF TYL352C0R3478,
    FKSTA TYPE VBUP-FKSTA,
    FKSAA TYPE VBUP-FKSAA,
  END OF TYL352C0R3478.
  DATA LML352C0R8306 TYPE TYL352C0R3478.
  LML352C0R8306-FKSTA = LSL352C0R387-FKSTA.
  LML352C0R8306-FKSAA = LSL352C0R387-FKSAA.
  MOVE-CORRESPONDING LML352C0R8306 TO LS_VBUP.
  SY-DBCNT = 1.
ELSE.
  SY-SUBRC = 4.
  SY-DBCNT = 0.
...
DATA LTL380C0R9337 TYPE TABLE OF VBUP.
CALL FUNCTION 'SD_VBUP_READ_FROM_DOC'
  EXPORTING
    I_VBELN             = '0000000001'
  IMPORTING
    ET_VBUP             = LTL380C0R9337
  EXCEPTIONS
    VBELN_NOT_FOUND     = 1
    VBTYP_NOT_SUPPORTED = 2
    VBOBJ_NOT_SUPPORTED = 3
    OTHERS              = 4.
IF SY-SUBRC = 0 AND LINES( LTL380C0R9337 ) > 0.
  CLEAR LT_VBUP.
  TYPES: BEGIN OF TYL380C0R68,
    VBELN TYPE VBUP-VBELN,
    POSNR TYPE VBUP-POSNR,
  END OF TYL380C0R68.
  DATA: LML380C0R1451 TYPE TYL380C0R68,
        LWL380C0R8945 LIKE LINE OF LT_VBUP.
  LOOP AT LTL380C0R9337 REFERENCE INTO DATA(LDRL380C0R2611).
    LML380C0R1451-VBELN = LDRL380C0R2611->VBELN.
    LML380C0R1451-POSNR = LDRL380C0R2611->POSNR.
    MOVE-CORRESPONDING LML380C0R1451 TO LWL380C0R8945.
    APPEND LWL380C0R8945 TO LT_VBUP.
  ENDLOOP.
  SY-DBCNT = LINES( LTL380C0R9337 ).
ELSE.
  SY-SUBRC = 4.
  SY-DBCNT = 0.
ENDIF.
...
DATA LTL390C0R3223 TYPE TABLE OF VBUP.
CALL FUNCTION 'SD_VBUP_READ_FROM_DOC'
  EXPORTING
    I_VBELN             = '0000000001'
  IMPORTING
    ET_VBUP             = LTL390C0R3223
  EXCEPTIONS
    VBELN_NOT_FOUND     = 1
    VBTYP_NOT_SUPPORTED = 2
    VBOBJ_NOT_SUPPORTED = 3
    OTHERS              = 4.
IF SY-SUBRC = 0 AND LINES( LTL390C0R3223 ) > 0.
  TYPES: BEGIN OF TYL390C0R5171,
    VBELN TYPE VBUP-VBELN,
    POSNR TYPE VBUP-POSNR,
  END OF TYL390C0R5171.
  DATA: LML390C0R3909 TYPE TYL390C0R5171,
        LWL390C0R2452 LIKE LINE OF LT_VBUP.
  LOOP AT LTL390C0R3223 REFERENCE INTO DATA(LDRL390C0R4166).
    LML390C0R3909-VBELN = LDRL390C0R4166->VBELN.
    LML390C0R3909-POSNR = LDRL390C0R4166->POSNR.
    MOVE-CORRESPONDING LML390C0R3909 TO LWL390C0R2452.
    APPEND LWL390C0R2452 TO LT_VBUP.
  ENDLOOP.
  SY-DBCNT = LINES( LTL390C0R3223 ).
ELSE.
  SY-SUBRC = 4.
  SY-DBCNT = 0.
ENDIF.
...
DATA LTL394C0R2896 TYPE TABLE OF VBUP.
CALL FUNCTION 'SD_VBUP_READ_FROM_DOC'
  EXPORTING
    I_VBELN             = '0000000001'
  IMPORTING
    ET_VBUP             = LTL394C0R2896
  EXCEPTIONS
    VBELN_NOT_FOUND     = 1
    VBTYP_NOT_SUPPORTED = 2
    VBOBJ_NOT_SUPPORTED = 3
    OTHERS              = 4.
IF SY-SUBRC = 0 AND LINES( LTL394C0R2896 ) > 0.
  APPEND LINES OF LTL394C0R2896 TO LT_VBUP.
  SY-DBCNT = LINES( LTL394C0R2896 ).
ELSE.
  SY-SUBRC = 4.
  SY-DBCNT = 0.
ENDIF.
...
LOOP AT LT_KEY REFERENCE INTO DATA(LDL401C0R3929).
  INSERT VALUE #( VBELN = LDL401C0R3929->* ) INTO TABLE LTL401C0.
ENDLOOP.
...
LOOP AT LT_KEY REFERENCE INTO DATA(LDL405C0R6108).
  INSERT VALUE #( VBELN = LDL405C0R6108->* ) INTO TABLE LTL405C0.
ENDLOOP.
CALL FUNCTION 'SD_VBUP_READ_FROM_DOC_MULTI'
  EXPORTING
...
LOOP AT LT_KEY REFERENCE INTO DATA(LDL410C0R3973).
  INSERT VALUE #( VBELN = LDL410C0R3973->* ) INTO TABLE LTL410C0.
ENDLOOP.
CALL FUNCTION 'SD_VBUP_READ_FROM_DOC_MULTI'
  EXPORTING
...
IF SY-SUBRC = 0 AND LINES( LTRL410C0 ) > 0.
  CLEAR LT_VBUP.
  TYPES: BEGIN OF TYL410C0R8726,
    VBELN TYPE VBUP-VBELN,
    POSNR TYPE VBUP-POSNR,
  END OF TYL410C0R8726.
  DATA: LML410C0R5104 TYPE TYL410C0R8726,
        LWL410C0R1241 LIKE LINE OF LT_VBUP.
  LOOP AT LTRL410C0 REFERENCE INTO DATA(LDRL410C0R7802).
    LML410C0R5104-VBELN = LDRL410C0R7802->VBELN.
    LML410C0R5104-POSNR = LDRL410C0R7802->POSNR.
    MOVE-CORRESPONDING LML410C0R5104 TO LWL410C0R1241.
    APPEND LWL410C0R1241 TO LT_VBUP.
  ENDLOOP.
...
LOOP AT LT_VBUK REFERENCE INTO DATA(LDL415C0R5086).
  INSERT VALUE #( VBELN = LDL415C0R5086->VBELN ) INTO TABLE LTL415C0.
ENDLOOP.
CALL FUNCTION 'SD_VBUP_READ_FROM_DOC_MULTI'
  EXPORTING
...
LOOP AT LT_KEYS REFERENCE INTO DATA(LDL424C0R1795).
  INSERT VALUE #( VBELN = LDL424C0R1795->VBELN
                  POSNR = LDL424C0R1795->POSNR ) INTO TABLE LTL424C0.
ENDLOOP.
CALL FUNCTION 'SD_VBUP_READ_FROM_DOC_MULTI'
  EXPORTING
...
LOOP AT LT_VBUP REFERENCE INTO DATA(LDL443C0R7185).
  INSERT VALUE #( VBELN = LDL443C0R7185->VBELN ) INTO TABLE LTL443C0.
ENDLOOP.
CALL FUNCTION 'SD_VBUP_READ_FROM_DOC_MULTI'
  EXPORTING
...
IF SY-SUBRC = 0 AND LINES( LTRL443C0 ) > 0.
  CLEAR LT_VBUP.
  TYPES: BEGIN OF TYL443C0R5940,
    MANDT TYPE VBUP-MANDT,
    VBELN TYPE VBUP-VBELN,
    POSNR TYPE VBUP-POSNR,
    RFSTA TYPE VBUP-RFSTA,
    FKIVP TYPE VBUP-FKIVP,
  END OF TYL443C0R5940.
  DATA: LML443C0R3779 TYPE TYL443C0R5940,
        LWL443C0R2351 LIKE LINE OF LT_VBUP.
  LOOP AT LTRL443C0 REFERENCE INTO DATA(LDRL443C0R8818).
    LML443C0R3779-MANDT = LDRL443C0R8818->MANDT.
    LML443C0R3779-VBELN = LDRL443C0R8818->VBELN.
    LML443C0R3779-POSNR = LDRL443C0R8818->POSNR.
    LML443C0R3779-RFSTA = LDRL443C0R8818->RFSTA.
    LML443C0R3779-FKIVP = LDRL443C0R8818->FKIVP.
    LWL443C0R2351 = LML443C0R3779.
    APPEND LWL443C0R2351 TO LT_VBUP.
  ENDLOOP.
...
LOOP AT LT_VBUP REFERENCE INTO DATA(LDL448C0R8336).
  INSERT VALUE #( VBELN = LDL448C0R8336->VBELN
                  POSNR = LDL448C0R8336->POSNR ) INTO TABLE LTL448C0.
ENDLOOP.
CALL FUNCTION 'SD_VBUP_READ_FROM_DOC_MULTI'
  EXPORTING
...
LOOP AT LT_KEYS REFERENCE INTO DATA(LDL453C0R3843).
  INSERT VALUE #( VBELN = LDL453C0R3843->VBELN ) INTO TABLE LTL453C0.
ENDLOOP.
CALL FUNCTION 'SD_VBUP_READ_FROM_DOC_MULTI'
  EXPORTING
...
IF SY-SUBRC = 0 AND LINES( LTRL453C0 ) > 0.
  ...
  TYPES: BEGIN OF TYL453C0R1067,
    VBELN TYPE VBUP-VBELN,
    POSNR TYPE VBUP-POSNR,
  END OF TYL453C0R1067.
  DATA: LML453C0R8376 TYPE TYL453C0R1067,
        LWL453C0R1304 LIKE LINE OF LT_SMALL_RESULT.
  LOOP AT LTRL453C0 REFERENCE INTO DATA(LDRL453C0R20).
    LML453C0R8376-VBELN = LDRL453C0R20->VBELN.
    LML453C0R8376-POSNR = LDRL453C0R20->POSNR.
    MOVE-CORRESPONDING LML453C0R8376 TO LWL453C0R1304.
    APPEND LWL453C0R1304 TO LT_SMALL_RESULT.
  ENDLOOP.
...
DATA LTL467C0R5273 TYPE TABLE OF VBUP.
CALL FUNCTION 'SD_VBUP_READ_FROM_DOC'
  EXPORTING
    I_VBELN             = '0000000001'
  IMPORTING
    ET_VBUP             = LTL467C0R5273
  EXCEPTIONS
    VBELN_NOT_FOUND     = 1
    VBTYP_NOT_SUPPORTED = 2
    VBOBJ_NOT_SUPPORTED = 3
    OTHERS              = 4.
IF SY-SUBRC = 0 AND LINES( LTL467C0R5273 ) > 0.
  LT_SORTED_RESULT = LTL467C0R5273.
  SY-DBCNT = LINES( LTL467C0R5273 ).
ELSE.
  SY-SUBRC = 4.
  SY-DBCNT = 0.
ENDIF.
...
DATA LTL473C0R4280 TYPE TABLE OF VBUP.
CALL FUNCTION 'SD_VBUP_READ_FROM_DOC'
  EXPORTING
    I_VBELN             = '0000000001'
  IMPORTING
    ET_VBUP             = LTL473C0R4280
  EXCEPTIONS
    VBELN_NOT_FOUND     = 1
    VBTYP_NOT_SUPPORTED = 2
    VBOBJ_NOT_SUPPORTED = 3
    OTHERS              = 4.
IF SY-SUBRC = 0 AND LINES( LTL473C0R4280 ) > 0.
  INSERT LINES OF LTL473C0R4280 INTO TABLE LT_SORTED.
  SY-DBCNT = LINES( LTL473C0R4280 ).
ELSE.
  SY-SUBRC = 4.
  SY-DBCNT = 0.
ENDIF.
...
DATA LTL479C0R2162 TYPE TABLE OF VBUP.
CALL FUNCTION 'SD_VBUP_READ_FROM_DOC'
  EXPORTING
    I_VBELN             = '0000000001'
  IMPORTING
    ET_VBUP             = LTL479C0R2162
  EXCEPTIONS
    VBELN_NOT_FOUND     = 1
    VBTYP_NOT_SUPPORTED = 2
    VBOBJ_NOT_SUPPORTED = 3
    OTHERS              = 4.
IF SY-SUBRC = 0 AND LINES( LTL479C0R2162 ) > 0.
  TYPES: BEGIN OF TYL479C0R159,
    VBELN TYPE VBUP-VBELN,
    POSNR TYPE VBUP-POSNR,
  END OF TYL479C0R159.
  DATA: LML479C0R7826 TYPE TYL479C0R159,
        LWL479C0R859 LIKE LINE OF LT_SORTED4.
  LOOP AT LTL479C0R2162 REFERENCE INTO DATA(LDRL479C0R4134).
    LML479C0R7826-VBELN = LDRL479C0R4134->VBELN.
    LML479C0R7826-POSNR = LDRL479C0R4134->POSNR.
    MOVE-CORRESPONDING LML479C0R7826 TO LWL479C0R859.
    INSERT LWL479C0R859 INTO TABLE LT_SORTED4.
  ENDLOOP.
  SY-DBCNT = LINES( LTL479C0R2162 ).
ELSE.
  SY-SUBRC = 4.
  SY-DBCNT = 0.
ENDIF.
...
LOOP AT LT_1234 REFERENCE INTO DATA(LDL490C0R383).
  INSERT VALUE #( VBELN = LDL490C0R383->VBELN ) INTO TABLE LTL490C0.
ENDLOOP.
CALL FUNCTION 'SD_VBUP_READ_FROM_DOC_MULTI'
  EXPORTING
